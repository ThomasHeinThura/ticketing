...
cookie authenticates the upgrade.

```
Client                          Server
  │  GET /ws  (cookie)            │
  ├──────────────────────────────►│  resolve session → identity
  │                               │  reject if no session (401)
...
**Subscription is authorized.** A client asking for `project:xyz` it cannot read is
refused. Broadcast is not a way around policy.
...
| `ws-subscribe-policy.test.ts` | Subscribing to an out-of-reach project is refused |
