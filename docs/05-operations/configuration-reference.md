...
| Amount | Five required; nine optional per-process switches; `TASKDESK_BOOTSTRAP_ADMIN_EMAIL` for headless installs; the three `POSTGRES_*` variables are read by the Postgres image only | Everything else |
