...
**Session log, 2026-09-23:** Continued #324 Slice 2b after two independent ordinary reviewers found the same stale-authorization-evidence class at exact head `566973b`. Every new shared authorization decision now resets legacy evidence to `unknown`; the bulk-task and ownership-transfer controllers record their post-middleware decision explicitly. Regression coverage exercises an authorization lookup error after an earlier allowed gate and an instance-admin bulk membership denial. The full unit suite passed across 12 tasks (API 484 tests; web 277 tests), `pnpm test:permissions` passed 10 files / 80 tests, API typecheck passed, focused shadow integration passed 12/12, lint passed 8/8, review and vocabulary checks passed, and `git diff --check` is clean. These changes are awaiting commit and fresh exact-head ordinary reviews; mandatory Opus review remains pending because `claude auth status` reports logged out. #324 remains draft and cannot close: project/work-item reach facts, portal mapping and deployed-traffic evidence are still missing.
