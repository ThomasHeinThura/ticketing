...
**2026-09-24 orchestrator snapshot — `main` at `f22f010`.** Merged today, each with every
...
Storybook 10.6.0). `protect-main` now requires **15** checks with strict up-to-date branches,
so each merge makes every other candidate BEHIND; a clean merge from `main` then needs a fresh
Opus merge-head attestation before the review-note binding passes again. Open follow-ups:
#357 (S9/S10 from #355). Everything else open is waiting on lane fixes or an unfilled PR
record, not on review capacity — check `gh pr list` for current state.
