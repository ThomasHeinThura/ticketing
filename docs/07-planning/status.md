...
**Seventh pass, 2026-09-23. `main` at `5ada9c5` (PR #345 merged).** P0 Foundation remains open. #317's timing-oracle candidate incorporates the DB-role split on main; its focused PostgreSQL 18 oracle tests passed for the session path (4/4 in the original file) and the API-key reach predicate (1/1), with both foreign and missing label lookups using one query. The full API integration suite passed 84 files / 1,144 tests before the API-key case was added; one subsequent full run had 1,144 existing tests pass and the newly added case fail only in the test's query-spy filter, then the corrected focused API-key case passed. Run the full suite again on the final candidate. #317's fresh ordinary review found no blocking code defect and called out the API-key/bulk evidence gap; this candidate now covers the API-key lookup, while bulk-specific coverage is still not demonstrated. #323 keeps #322's applied migration `0068_workspace_role_is_system` at index 68 and moves #8 Slice 2's shadow-table migration to `0069_policy_shadow_tables` at index 69, with the snapshot chained from #322's snapshot. Its focused PostgreSQL 18 shadow integration test passed 10/10; lint, typecheck, permissions (80), route-policy, OpenAPI (106 operations), review-spec, vocabulary (59), env and full unit retry passed. Two fresh ordinary reviews and one delta review found no blocking #323 issue; the stale snapshot-lineage comment was fixed and delta-reviewed. Both candidates still require independent Opus security review before merge. No P0 gate is claimed closed; #8 still lacks complete route enforcement and coverage, and #324 plus other prerequisite lanes remain open. Re-verify live PR/issue state before dispatch.
...
**#317 S4 coverage follow-up, 2026-09-23:** A fresh ordinary review found the S4 integration test exercised only session authentication. Added a valid API-key test that compares foreign/missing responses and checks the lookup SQL contains both workspace membership and API-key validity predicates; focused test and API typecheck passed. The first full integration attempt reported 1 failure / 1,144 passes because the new spy assumed a string SQL argument and uppercase SQL text; fixed the spy to support pg query-config objects and case-normalized matching, then the focused API-key case passed. Full integration rerun on this candidate remains necessary; bulk-specific query proof remains an unclosed evidence gap. Opus final review is still mandatory.
