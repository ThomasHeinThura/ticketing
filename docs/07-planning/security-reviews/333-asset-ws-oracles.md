# Security review — #317 S3: asset and websocket existence oracles (PR #333 / #338)

**Reviewer:** Opus 5.5, fresh independent context. Did not author, direct, or remediate this change.
**Reviewed head:** `896cec2c58b1c9014aa146a068642e8ef2fa9bc0`
**Reviewed SHA:** `896cec2c58b1c9014aa146a068642e8ef2fa9bc0` (confirmed via `gh pr view 333 --json headRefOid`)
**Same tree as:** `59176784b4717f3ebd35c08cef6f63b83c78a2f9`, the head of PR #338. `git rev-parse <sha>^{tree}` gives `f9a5b5fa35bb163c5c7cea56553b05f4b7162879` for both, and `git diff 896cec2 5917678` is empty. Everything below applies byte-for-byte to #338's tree. It does **not** replace an exact-head attestation on #338. That PR's reviewed-head binding still has to name `5917678…` (or whatever head it ends up with).
**Base:** `dd067e21f77853b35dd79de31e99d258678b92b4` (`origin/main` at review time; merge-base equals it)
**Branch:** `fix/317-oracle-hardening`
**Pull request:** #333. It was **closed** at 2026-09-23T16:30Z and superseded by #338 (`fix/317-oracle-clean`) because of a gitleaks hit in its first commit.
**Implemented by (as recorded in the PR body):** "Codex agent; exact model variant is not exposed in this runtime". The commits are authored `Claude Code`, but the PR's attribution comment says a non-Claude lane agent wrote them.
**Date:** 2026-09-23
...
- **Ordinary independent review.** The PR body's `## Reviewed by` claims an "Independent Codex reviewer context (exact model variant is not exposed)", session `release_delta_review`, CLEAR at `896cec2…`.
  - It names **no model**.
  ...
  - On #338, `## Reviewed by` is "Pending fresh review of the clean candidate head".
...
- **Security review.** This note. It must still be bound to #338's exact head before that PR merges.

## Verification run (at `896cec2`, same tree as `5917678`)
...
- I did not review #338's body or its exact-head gates beyond confirming its tree is identical to #333's.
