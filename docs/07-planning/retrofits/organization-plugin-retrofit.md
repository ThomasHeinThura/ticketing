...
| **S7** — native role list and writes | ⛔ **BLOCKED BY #82 AND #118** | — | **#66 is closed**, fixed by PR #80 (squash `6e6c9ae`) and verified on `main`: a missing `workspace_role` row is now a DENY for every role but `owner`, whose authority is compiled-in by design (retrofit plan R5). The privilege-restoration fail-open on the very table S7 writes is therefore gone. **Two blockers remain, and a third documentation-shaped one.** **(a) #82** — one membership = exactly one role, where multi-role values diverge between the two evaluators (see the decision log). **(b) #118** — `workspace_role` carries no `UNIQUE (workspace_id, role)` (`data-model`-level defect measured against real PostgreSQL: duplicate rows for one `(workspace_id, role)` pair with *different* `permission` payloads insert cleanly, reachable through the plugin's `create-role`, and both `customRoleStatements` and `ownRoleStatements` read them with an unordered `LIMIT 1`). S7 adds the first native write path to that table, so the constraint must land with it — an evaluator that guesses between two privilege definitions is exactly the class of defect #66 closed one table over. **(c)** `roles-and-permissions-ui.md` still has open review findings and `pnpm check:reviews` refuses to let a feature whose review section is non-empty proceed; see the S7 blueprint's "third prerequisite" section, which treats closing them as step 0. Check GitHub for each issue's current state rather than trusting this row. **SCOPE WIDENED, 2026-09-10:** S7 also owns the native role **list** route and the repointing of all four remaining client callers — `listRoles`, `createRole`, `updateRole`, `deleteRole`. The original row scoped S7 to the three write verbs only, which would have retired three of the four callers and left the `listRoles` **read** live, leaving S10's zero-caller precondition unmet by exactly one family. A review of PR #99 caught that. **S7's exit criterion is zero executable `authClient.organization.*` callers in `apps/web/src`, not "routes implemented".** |
...
- **S7** — ⛔ **BLOCKED BY #82 AND #118.** #66 is closed (PR #80), so the privilege-restoration
  fail-open on `workspace_role` — the exact table S7 writes — is fixed and no longer blocks it.
  Two blockers remain. **#82**: one membership = exactly one role, where multi-role values
  diverge between the two evaluators. **#118**: `workspace_role` has no
  `UNIQUE (workspace_id, role)`, so two rows for one pair can carry *different* permission sets
  and both evaluators pick between them with an unordered `LIMIT 1`; S7 is the stage that adds
  the first native write path to that table. Neither is a scheduling preference — authoring role
  writes first would build on two known privilege defects at once. A third prerequisite is
  documentation-shaped: `roles-and-permissions-ui.md`'s review section must be closed before the
  feature may be implemented (AGENTS.md do-not 15, enforced by `pnpm check:reviews`).
