...
| **S3** — client reads move off the plugin | 🔄 **IN FLIGHT** | — | Reads repointed at the S2 routes. Check GitHub for the current branch/PR and its state — this row does not track it. **Must not merge before S5 lands:** after S3 no client read returns the `workspace_member` row id that the two still-on-plugin mutations (`updateMemberRole`, the promote/demote transfer) require, so both are force-disabled on that branch until S5's userId-keyed routes land |
...
| **S4b** — client workspace-write cutover | ❌ **NOT STARTED** | — | Repoint `authClient.organization.create` / `.update` / `.delete` — six client call sites (`apps/web/src/fetchers/workspace/{create,update,delete}-workspace.ts`, `apps/web/src/hooks/queries/workspace/use-create-workspace.ts`, `apps/web/src/hooks/mutations/workspace/use-{update,delete}-workspace.ts`) — onto the native S4 routes, already on `main`. Not S5/S6a/S7/S8a's job: none of their written scopes covers workspace CRUD, and S4's own row says this ships dark. **Blocks S9 and S10.** |
| **S5** — native membership writes | 🔄 **IN FLIGHT** | — | Five routes plus an atomic ownership transfer; in security-review scope. Check GitHub for the current branch/PR and its review/merge state — this row does not track it. **Merge before S3** — see the S3 row above |
...
| **S7** — native role writes | ⛔ **BLOCKED BY #66** | — | S4 is satisfied, but #66 — `hasWorkspacePermission` falls back to compiled built-in roles when a `workspace_role` row is absent — is a privilege-restoration fail-open on the very table S7 writes. **Do not author native role-delete routes until #66 is merged and independently cleared.** Independently, also blocked by #82 (one membership = exactly one role; see the decision log) — check GitHub for both issues' state. |
...
| **S10** — unmount (the tripwire commit) | ❌ **NOT STARTED** | — | Needs S3, S4b and S5–S9 all merged. `tests/api-contract/openapi.json` still declares six `/auth/organization/*` invitation operations, which is exactly what S10 removes |
...
**"Four of fifteen" is a misleading denominator** and should not be quoted on its own: it
...
| **Required to reach S10, outstanding** | S3, S4b, S5, S6a, S7, S8a, S9, S10 | **8** |
...
So the live figure is **4 landed of 12 required**, with **8 outstanding**, and S10 last
because everything else feeds it. (**S4b is new to this count** — it was always required
work, just not previously written down as its own row; see the S4b row.)
...
- **S3** — in flight; check GitHub for the branch/PR and its state. Must not merge before S5:
  see the S3 row.
- **S4b** — startable now (S4 is merged). Blocks S9 and S10.
- **S5** — in flight; check GitHub for the branch/PR and its state. In security-review scope.
- **S7** — ⛔ **BLOCKED BY #66**, and independently by #82. Not a scheduling preference: #66 is a fail-open on
