...
- **S7** — ⛔ **BLOCKED BY #82 AND #118** (corrected 2026-09-15). #66 is closed (PR #80), so
  the fail-open on `workspace_role` — the exact table S7 writes — is fixed and no longer
  blocks it. Two blockers remain: **#82** — one membership = exactly one role, where
  multi-role values diverge between the two evaluators (PR #110); **#118** —
  `workspace_role` has no `UNIQUE (workspace_id, role)`, so duplicate rows can disagree on
  the same table S7 writes (PRs #119, #122). Not a scheduling preference — authoring role
  writes first would build on two known privilege defects at once.
