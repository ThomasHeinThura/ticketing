...
| **S3** — client reads move off the plugin | ✅ **COMPLETE** | PR #76, squash `3e78450a5` | Reads repointed off the plugin onto the S2 routes. The two mutations S3 had force-disabled — `updateMemberRole` and the promote/demote ownership transfer — are repointed at S5's userId-keyed routes and **re-enabled**, and S4b's `refresh-workspace-stores` shim and its test are deleted, their job now done by native query invalidation. Out of security-review scope (all `apps/web`), so cleared by two independent Sonnet reviews plus a **manual browser pass** — every step verified against the database rather than the screen, after a first attempt showed the new name on screen while the database still held the old one |
...
| **S7** — native role list and writes | ✅ **UNBLOCKED, 2026-09-15** — not yet started | — | **#66 is closed**, fixed by PR #80 (squash `6e6c9ae`) and verified on `main`: a missing `workspace_role` row is now a DENY for every role but `owner`, whose authority is compiled-in by design (retrofit plan R5). The privilege-restoration fail-open on the very table S7 writes is therefore gone. **#82 is CLOSED** — PR #110 reconciled the two evaluators' disagreement on malformed multi-role values, independently reviewed 2 Sonnet + 1 Opus, CLEAR. **#118 is CLOSED** — PR #122 added `UNIQUE (workspace_id, role)` (migration `0051`) and PR #119 made both evaluators refuse rather than guess on a remaining ambiguity; both independently reviewed 2 Sonnet + 1 Opus, CLEAR, notes committed at `docs/07-planning/security-reviews/122-workspace-role-unique.md` and `119-workspace-role-evaluator.md`. **S7 has no remaining precondition.** **SCOPE, per the 2026-09-10 widening:** S7 also owns the native role **list** route and the repointing of all four remaining client callers — `listRoles`, `createRole`, `updateRole`, `deleteRole`. The original row scoped S7 to the three write verbs only, which would have retired three of the four callers and left the `listRoles` **read** live, leaving S10's zero-caller precondition unmet by exactly one family. A review of PR #99 caught that. **S7's exit criterion is zero executable `authClient.organization.*` callers in `apps/web/src`, not "routes implemented".** |
...
- **S7** — ✅ **UNBLOCKED, 2026-09-15** — not yet started. #66 is closed (PR #80), so the
  fail-open on `workspace_role` — the exact table S7 writes — is fixed. Both remaining
  blockers now closed: **#82** — one membership = exactly one role, where multi-role values
  diverge between the two evaluators — remediated by PR #110 (merged, 2 Sonnet + 1 Opus,
  CLEAR). **#118** — `workspace_role` has no `UNIQUE (workspace_id, role)` — remediated by
  PR #122 (database constraint) and PR #119 (evaluator fail-closed), both merged,
  independently reviewed, CLEAR. S7 has no remaining precondition.
...
with it. **That edge is now satisfied — S4b merged as PR #85 — so S9 is startable.**
...
| Set active workspace | `authClient.organization.setActive` | `apps/web/src/components/workspace-switcher.tsx:59`; `apps/web/src/components/onboarding/onboarding-flow.tsx:81`; `apps/web/src/components/shared/modals/create-workspace-modal.tsx:62`; `apps/web/src/routes/invitation/accept.$inviteId.tsx:62` — **and five more this row never listed; `grep -rn 'authClient\.organization\.setActive(' apps/web/src` returns nine. See #100** |
...
| **S7 — native role list and writes** | `GET/POST/PATCH/DELETE /api/workspace/{id}/roles` over `workspace_role` — the **read** included, because `listRoles` is one of the four client callers S7 must retire and S10's precondition is zero of them. **Corrected here (S7 blueprint Ambiguity Q5, formal review): this row previously bundled all three of `rbac.md:159-175`'s guardrails together as deferred. That was wrong for the first one.** `cannot grant a capability you do not hold` is `RL-3` in [`roles-and-permissions-ui.md`](../../03-features/roles-and-permissions-ui.md) — an already-accepted spec acceptance criterion (its review section closed via PR #128), not an open design question — and it needs no new vocabulary: it is a plain check of whether the calling member's own resolved statements already include what they are trying to grant, using the same `hasWorkspacePermission` resolution chain S4–S8a already call (`resolveCallerWorkspaceStatements`, `require-workspace-permission.ts`). **It IS implemented, in the create and update controllers.** Only the other two — **rank comparison** and the **last-administrator check** — genuinely remain deferred: both depend on `rank` / `is_system` vocabulary this legacy `workspace_role` shape has no columns for. **Unblocked, 2026-09-15** — #66 is closed (PR #80) and #82 AND #118 are both closed (PRs #110, #122, #119); see the main stage table. | S4 (seeding path settled). | S1 role assertions reproduced against the native routes in `tests/api-integration/workspace-role-writes.test.ts`; a test that editing `admin` in workspace A does not affect workspace B. | No |
