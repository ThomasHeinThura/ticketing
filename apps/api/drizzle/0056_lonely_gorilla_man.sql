...
--
-- FIX: every ancestor visited during the walk is read with `SELECT ... FOR UPDATE`, not
-- a plain SELECT -- a real row lock, held until the visiting transaction's own commit or
-- rollback. A concurrent transaction that is itself changing that ancestor's own
-- `parent_id` already holds that row's lock for the duration of its own UPDATE, so this
-- walk BLOCKS on it instead of reading a stale, about-to-change value: it can only
-- proceed once that other transaction has either committed (seeing its real, final
-- result) or rolled back (seeing the true, unchanged value) -- never a value mid-flight.
-- Applied to the 2-row race above: whichever of T1/T2 reaches the other's row first (via
-- this FOR UPDATE read) blocks the second until the first resolves, so the second
...
BEGIN
  ...
  LOOP
    ...
    -- Row lock, not a plain read -- see this function's header comment for why. Every
    -- ancestor actually on the FK-guaranteed chain exists, so `NOT FOUND` here would
    -- mean a dangling reference; fail closed rather than loop.
