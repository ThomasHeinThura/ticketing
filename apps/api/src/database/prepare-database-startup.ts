...
export async function prepareDatabaseStartup({
  ...
  try {
    ...
    throw new Error(
      `Database startup failed for ${config.host}:${config.port}/${config.database} (source: ${config.source}). If you are running outside Docker Compose, use localhost or set TASKDESK_DATABASE_URL explicitly.`,
      { cause: error },
    );
  }
