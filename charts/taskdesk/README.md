...
| `taskdesk.env.database.external.migration.enabled` | Point `TASKDESK_MIGRATION_DATABASE_URL` at a separate owner role for an external Postgres, read only by the `taskdesk-migrate` Job (issue #296). When `false`, that Job runs migrations as the same role as `taskdesk.env.database.external.*` above — this either works (if that role happens to have DDL rights) or the Job fails loudly at `helm install`/`upgrade`, since it is a pre-install/pre-upgrade hook | `false` |
...
| `migrateJob.enabled`                   | Run the `taskdesk-migrate` pre-install/pre-upgrade hook Job (issue #296, S1). The ONLY object in this chart that ever receives `TASKDESK_MIGRATION_DATABASE_URL` — the TaskDesk Deployment never does, and refuses to start if it ever finds that variable in its own environment | `true` |
| `migrateJob.backoffLimit`              | Job `backoffLimit`                                                                                                     | `2`                              |
| `migrateJob.resources`                 | Resource requests and limits for the migrate Job's container                                                           | `{}`                             |
