...
describe("getInvitationEmailSubject", () => {
  it("uses French copy for regional French locales", () => {
    ...
    expect(subject).toBe(
      "Alice vous invite à rejoindre Équipe produit sur TaskDesk",
    );
  });
  ...
  it("keeps the German translation unchanged", () => {
    ...
    expect(subject).toBe(
      "Alice hat dich eingeladen, Produkt auf TaskDesk beizutreten",
    );
  });
  ...
  it("uses Brazilian Portuguese copy for regional Portuguese locales", () => {
    ...
    expect(subject).toBe(
      "Alice convidou você para participar de Equipe produto no TaskDesk",
    );
  });
  ...
  it("uses Japanese copy for Japanese locales", () => {
    ...
    expect(subject).toBe(
      "Alice さんが TaskDesk の「プロダクト」にあなたを招待しました",
    );
  });
  ...
  it("uses the English fallback for unsupported locales", () => {
    ...
    expect(subject).toBe("Alice invited you to join Producto on TaskDesk");
  });
});
