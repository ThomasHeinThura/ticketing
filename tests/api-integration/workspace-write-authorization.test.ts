 ...
 *     membership precondition; see `require-workspace-membership.ts`. The
 *     residual bypass for an instance admin who IS a member is pinned below.
 *
...
describe("S4 native writes: the instance-admin boundary (A2-P16..A2-P17)", () => {
  ...
  // A2-P17. PINNED, NOT CLOSED, and flagged in the pull request.
  //
  ...
  // member passes the capability check whatever their role says. Under the
  // inherited plugin a `viewer` member could not update a workspace, instance
  // admin or not.
  //
  // This is pre-existing SHARED behaviour that every other authenticated route
  // in the product already depends on. Changing it is a decision about the
  // shared evaluator — #7's territory, adjacent to #66 — not something to
  // slip into a route batch. Asserted here so it is visible and cannot drift.
  it("A2-P17 FINDING (shared evaluator, unfixed here): an instance admin who is a VIEWER member still passes the capability check", async () => {
    const { app } = createApp();
    const instanceAdmin = await signUpUser(app);
    const owner = await signUpUser(app);
    ...
    // 200 is the shared evaluator's instance-admin bypass, not a decision
    // this batch made. If the bypass is ever narrowed, this becomes 403 and
